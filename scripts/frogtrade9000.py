...
def setup_client(name=None, config_path=None, url=None, port=None, username=None, password=None):
    ...
    print(
        f"Setting up {name} version {c['version']} at {server_url}: {strategy} {bot_state} {runmode}"
    )
    sleep(1)
...
def make_layout(
    exclude_charts=False,
    include_sysinfo=False,
    include_candle_info=False,
        ...
        if include_sysinfo:
            if include_candle_info:
                layout["right_side"].split(
                    Layout(name="chart1", ratio=2),
                    Layout(name="sys_info"),
                    Layout(name="candle_info"),
                )
            else:
...
def sysinfo(client_dict) -> Panel:
    ...
    for n, client in uniqclients.items():
        ...
        progress_cpu = Progress(
            "{task.description}",
            BarColumn(
                bar_width=None, complete_style=Style(color="red"), finished_style=Style(color="red")
            ),
        ...
        progress_table.add_row(progress_cpu, progress_ram)
    ...
    for n, client in client_dict.items():
        ...
        for tag, trades in trades_by_tag.items():
            ...
            for t in trades:
                ...
                t_profit += profit
                tdur = (
                    datetime.strptime(t["close_date"], dfmt)
                    - datetime.strptime(t["open_date"], dfmt)
                ).total_seconds()
...
def trades_summary(client_dict) -> Table:
    ...
    for n, client in client_dict.items():
        ...
        table.add_row(
            ...
            f"[red]{round(expectancy_rate, 2)}"
            if expectancy_rate <= 0
            else f"[green]{round(expectancy_rate, 2)}",
    ...
    table.columns[3].footer = (
        f"[red]{round(all_open_profit, 2)}"
        if all_open_profit <= 0
        else f"[green]{round(all_open_profit, 2)}"
    )
    table.columns[4].footer = f"[green]{all_wins}/[red]{all_losses}"
    table.columns[10].footer = (
    ...
    for n, client in client_dict.items():
        ...
        for t in trades:
            ...
            pairstr = (
                ...
                + (
                    "*"
                    if (t["open_order_id"] is not None and t["close_rate_requested"] is None)
                    else ""
                )
                + ("**" if (t["close_rate_requested"] is not None) else "")
            )
            ...
            if "buy_tag" in t.keys():
                table.add_row(
                    ...
                    f"[red]{t['profit_pct']}"
                    if t["profit_pct"] <= 0
                    else f"[green]{t['profit_pct']}",
                    f"[red]{round(t['profit_abs'], 2)}"
                    if t["profit_abs"] < 0
                    else f"[green]{round(t['profit_abs'], 2)}",
...
def pair_chart(basic_chart, height=20, width=120, limit=None, timeframe=None, basic_symbols=False):
    ...
    return (
        chart_config["current_pair"],
        basic_chart.get_chart_str(height=height, width=width, basic_symbols=basic_symbols),
    )
...
def profit_chart(basic_chart, all_trades, height=20, width=120, limit=None, basic_symbols=False):
    ...
    if limit is not None:
        basic_chart.set_limit(limit)
    return basic_chart.get_profit_str(
        all_trades, height=height, width=width, basic_symbols=basic_symbols
    )
...
def main():
    ...
    parser.add_argument(
        ...
        "--servers",
        nargs="?",
        help="If you have multiple servers or your config differs from the REST API server URLs, specify each one here with [<name>@]<url>:<port> separated by a comma, e.g. mybotname@my.server:8081,my.server:8082,192.168.0.69:8083",
    )
    parser.add_argument("-t", "--stake_coin", nargs="?", help="Stake coin. Default: USDT")
    parser.add_argument(
        "-i", "--informative_coin", nargs="?", help="Informative coin. Default: BTC"
    )
    parser.add_argument(
        "-b",
        "--basic_symbols",
        action="store_true",
        help="Display non-rounded ASCII charts, for TTYs with poor fancy symbol support. Default: False",
    )
    parser.add_argument(
        "-x",
        "--exclude_charts",
        action="store_true",
        help="Do not draw charts, and expand sidebar to whole window. Default: False",
    )
    parser.add_argument(
        "-f",
        "--include_sysinfo",
        action="store_true",
        help="Include system information. If charts are also excluded, this will take up the full right pane. If not, it will replace the profit chart. Default: False",
    )
    parser.add_argument(
        "-k",
        "--include_candle_info",
        action="store_true",
        help="Include 5m candle information. Default: False",
    )
    parser.add_argument(
        "-o",
        "--include_tag_summary",
        action="store_true",
        help="Include summary of entry tags. Default: False",
    )
            ...
            for s in slist:
                ...
                if m:
                    ...
                    if url is None or port is None:
                        raise Exception(
                            "Cannot get URL and port from server option. Please use [name]user:pass@servername:port"
                        )
                    ...
                    try:
                        if config is not None:
                            ...
                            name, client = setup_client(
                                name=botname, url=url, port=port, username=suser, password=spass
                            )
                        client_dict[name] = client
                    except Exception as e:
                        raise RuntimeError("Cannot create freqtrade client") from e
                    ...
                    raise Exception(
                        "Cannot parse server option. Please use [name]user:pass@servername:port"
                    )
    ...
    pc = bc.BasicCharts(
        symbol=chart_config["current_pair"], timeframe=chart_config["current_timeframe"], limit=cw
    )

    if args.exclude_charts and args.include_candle_info:
        layout = make_candle_info_layout(
            exclude_charts=args.exclude_charts,
            include_sysinfo=args.include_sysinfo,
            include_tag_summary=args.include_tag_summary,
            side_panel_minimum_size=side_panel_minimum_size,
            num_days_daily=num_days_daily,
        )
    else:
        layout = make_layout(
            exclude_charts=args.exclude_charts,
            include_sysinfo=args.include_sysinfo,
            include_candle_info=args.include_candle_info,
            include_tag_summary=args.include_tag_summary,
            side_panel_minimum_size=side_panel_minimum_size,
            num_days_daily=num_days_daily,
        )
    ...
    if args.include_sysinfo:
        layout["sys_info"].update(
            Panel(
                Status("Loading...", spinner="line"),
                title="[b]System Information",
                border_style="magenta",
            )
        )

    if args.include_candle_info:
        layout["candle_info"].update(
            Panel(
                Status("Loading...", spinner="line"),
                title="[b]Candle Information",
                border_style="cyan",
            )
        )

    if args.include_tag_summary:
        layout["tag_summary"].update(
            Panel(
                Status("Loading...", spinner="line"),
                title="[b]Entry Tag Summary",
                border_style="white",
            )
        )

    layout["open"].update(
        Panel(Status("Loading...", spinner="line"), title="Open Trades", border_style="green")
    )
    layout["summary"].update(
        Panel(Status("Loading...", spinner="line"), title="Trades Summary", border_style="red")
    )
    layout["daily"].update(
        Panel(Status("Loading...", spinner="line"), title="Daily Profit", border_style="yellow")
    )
    layout["closed"].update(
        Panel(Status("Loading...", spinner="line"), title="Closed Trades", border_style="blue")
    )

    layout["footer_clock"].update(
        datetime.now(tz=timezone.utc).ctime().replace(":", "[blink]:[/]") + " UTC"
    )
    layout["footer_left"].update(" | Status: Loading...")
    layout["footer_right"].update(
        Text("frogtrade9000 by @froggleston [https://github.com/froggleston]", justify="right")
    )
            ...
            while True:
                try:
                    ...
                    if do_info_panels_update:
                        ...
                        layout["summary"].update(
                            Panel(
                                trades_summary(client_dict),
                                title="Trades Summary",
                                border_style="red",
                                height=7 + len(client_dict.items()),
                            )
                        )

                        layout["daily"].update(
                            Panel(
                                daily_profit_table(client_dict, num_days_daily),
                                title="Daily Profit",
                                border_style="yellow",
                                height=(num_days_daily + 6),
                            )
                        )
                        layout["closed"].update(
                            Panel(
                                closed_trades_table(
                                    client_dict, all_closed_trades, num_closed_trades
                                ),
                                title="Closed Trades",
                                border_style="blue",
                            )
                        ...
                        if not args.exclude_charts:
                            spc = pair_chart(
                                ...
                                width=cw,
                                limit=cw,
                                timeframe=chart_config["current_timeframe"],
                                basic_symbols=args.basic_symbols,
                            )

                            layout["chart1"].update(
                                Panel(spc[1], title=f"{spc[0]} [{pc.get_timeframe()}]")
                            )

                            if not args.include_sysinfo:
                                ppc = profit_chart(
                                    pc,
                                    all_closed_trades[chart_config["current_summary"]],
                                    height=ch - 4,
                                    width=cw,
                                    basic_symbols=args.basic_symbols,
                                )
                                layout["chart2"].update(
                                    Panel(
                                        ppc,
                                        title=f"{chart_config['current_summary']} Cumulative Profit",
                                    )
                                )
                            else:
                                if args.include_candle_info:
                                    layout["candle_info"].update(
                                        Panel(
                                            tradeinfo(client_dict, all_closed_trades, indicators),
                                            title="Recent Buy Info",
                                            border_style="cyan",
                                        )
                                    )
                        else:
