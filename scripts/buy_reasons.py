...
def do_trade_buys(
    pair, data, trades, alt_tag=None, pickled_signal_candles=None, strategy_version=3
):
...
def do_group_table_output(bigdf, glist):
    if "0" in glist:
        ...
        new = bigdf.groupby(["enter_reason"]).agg(
            {
                "profit_abs": [
                    "count",
                    lambda x: sum(x > 0),
    ...
    if "2" in glist:
        new = (
            bigdf.groupby(["enter_reason", "exit_reason"])
            .agg(
                {
                    "profit_abs": ["count", "sum", "median", "mean"],
                    "profit_ratio": ["sum", "median", "mean"],
    ...
    if "3" in glist:
        new = (
            bigdf.groupby(["pair", "enter_reason"])
            .agg(
                {
                    "profit_abs": ["count", "sum", "median", "mean"],
                    "profit_ratio": ["sum", "median", "mean"],
    ...
    if "4" in glist:
        new = (
            bigdf.groupby(["pair", "enter_reason", "exit_reason"])
            .agg(
                {
                    "profit_abs": ["count", "sum", "median", "mean"],
                    "profit_ratio": ["sum", "median", "mean"],
...
def process_one(
    ...
    verbose,
    write_out,
    use_timeframe_detail,
    pickled_signal_candles,
):
    ...
    for sname in stratnames:
        ...
        ft_exchange = ExchangeResolver.load_exchange(
            loc_config["exchange"]["name"], config=loc_config, validate=False
        )
    ...
    if parallel:
        ...
        while i < len(buy_reason_jobs):
            ...
            with Pool(max(job_split, 1)) as pool:
                results.extend(
                    [job.get() for job in [pool.apply_async(process_one, p) for p in tasks]]
                )
                pool.close()
                pool.join()
            i += len(tasks)
...
def main():
    ...
    parser.add_argument(
        "--indicator_list", nargs="?", help="Comma separated list of indicators to analyse"
    )
    parser.add_argument(
        "--enter_reason_list",
        nargs="?",
    ...
    parser.add_argument(
        "-p", "--pairlist", nargs="?", help="pairlist as 'BTC/USDT,FOO/USDT,BAR/USDT'"
    )
    parser.add_argument(
        "-u",
        "--use_trades_db",
    ...
    parser.add_argument(
        "-w", "--write_out", help="write an output CSV per pair", action="store_true"
    )
    parser.add_argument(
        "-g",
        "--group",
    ...
    parser.add_argument(
        "-o", "--outfile", help="write all trades summary table output", type=argparse.FileType("w")
    )
    parser.add_argument(
        "-d",
        "--data_format",
    ...
    parser.add_argument(
        ...
        "--indicators",
        nargs="?",
        help="Indicator values to output, as a comma separated list, " "e.g. -i rsi,adx,macd",
    )
    parser.add_argument(
        "-x", "--cancels", action="store_true", help="Output buy cancel reasons. Default: False"
    )
    parser.add_argument(
        "-a",
    ...
    if args.signal_candles_pickle is not None:
        ...
        scpf = Path(
            backtest_dir,
            os.path.splitext(get_latest_backtest_filename(backtest_dir))[0] + "_signals.pkl",
        )
        scp = open(scpf, "rb")
        pickled_signal_candles = joblib.load(scp)
    ...
    print_results(
        ...
        args.outfile,
        args.group,
        args.enter_reason_list,
        args.exit_reason_list,
        args.indicator_list,
        args.cancels,
    )
        ...
        if bigdf.shape[0] > 0 and ("enter_reason" in bigdf.columns):
            ...
            if indicator_list is not None:
                if indicator_list == "all":
                    ...
                    print(
                        tabulate(
                            bigdf[ilist].sort_values(["exit_reason"]),
                            headers="keys",
                            tablefmt="psql",
                            showindex=False,
                        )
                ...
                print(
                    tabulate(
                        bigdf[columns].sort_values(["pair"]),
                        headers="keys",
                        tablefmt="psql",
                        showindex=False,
                    )
